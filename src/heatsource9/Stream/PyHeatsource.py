...
def safe_divide(numerator, denominator):
    if denominator == 0:
        logging.error("Division by zero attempted with numerator: %s", numerator)
        return None  # Or handle as appropriate
    else:
        return numerator / denominator


def calc_solar_position(lat, lon, hour, minute, sec, offset,
    ...
    SolarTime = ((hour*60.0) + minute + (sec/60.0) +
...
def get_ground_fluxes(cloud, wind, humidity, T_air, elevation, phi,
    ...
    #     F_LW_Stream = -0.96 * Sigma * (T_prev + 273.2) ** 4
    # except OverflowError:
    #     print ('overflow error, T_prev: %s'%(T_prev))
    #     sys.exit()
